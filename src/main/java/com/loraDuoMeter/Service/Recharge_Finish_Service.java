...
@Service
public class Recharge_Finish_Service {

    private final IRecharge_Finish_Repo repo;

    public Recharge_Finish_Service(IRecharge_Finish_Repo repo) {
        this.repo = repo;
    }

    @Transactional(readOnly = true)
    public List<RechargeOverDisplay_DTO> getLatestByDevEui() {
        try (Stream<IRechargeOverDisplayView> stream = repo.findLatestByDevEuiNative()) {
            return stream.map(p -> new RechargeOverDisplay_DTO(
                p.getSlNo(),
                p.getNetworkType(),
                p.getDeviceType(),
    ...
    @Transactional(readOnly = true)
    public Slice<RechargeOverDisplay_DTO> getByDevEui(
