...
public class TamperController {
	...
	private Main_Service mainService;
	 ...
	 public List<DailyUpdateDisplay_DTO> fetchLatestByDevEui() {
		...
		/*
		 * @GetMapping("/get-tamper-events") public ResponseEntity<?>
		 * getAllTamperEvents() { try { // 2. Fetch all required data
		 * List<TamperEventEntity> events = tamperRepo.findAllByOrderBySlNoAsc();
		 * List<MeterEntity> meters = meterRepo.findAll(); List<BuildingEntity>
		 * buildings = buildingRepo.findAll();
		 * 
		 * // 3. Create a Map for Buildings for faster lookup (Key: BuildingID)
		 * Map<String, BuildingEntity> buildingMap = buildings.stream() .filter(b ->
		 * b.getBuildingId() != null)
		 * .collect(Collectors.toMap(BuildingEntity::getBuildingId, b -> b, (b1, b2) ->
		 * b1));
		 * 
		 * List<TamperEventDto> filteredEvents = new ArrayList<>();
		 * 
		 * for (TamperEventEntity event : events) { String tamperDevEui =
		 * event.getDevEui();
		 * 
		 * if (tamperDevEui != null) { // 4. Find matching Meter Optional<MeterEntity>
		 * matchingMeter = meters.stream() .filter(m -> m.getSerialNo() != null &&
		 * m.getSerialNo().equalsIgnoreCase(tamperDevEui)) .findFirst();
		 * 
		 * if (matchingMeter.isPresent()) { MeterEntity meter = matchingMeter.get();
		 * 
		 * // 5. Strict Check: Must have Building ID if (meter.getBuildingId() != null
		 * && !meter.getBuildingId().trim().isEmpty()) {
		 * 
		 * TamperEventDto dto = new TamperEventDto();
		 * 
		 * // A. Map Tamper Info dto.setSlNo(event.getSlNo());
		 * dto.setDeviceType(event.getDeviceType()); dto.setDevEui(event.getDevEui());
		 * dto.setTamperType(event.getTamperType());
		 * dto.setEventTime(event.getReceivedAt());
		 * dto.setLastInsertedDate(event.getLastUpdateTime());
		 * 
		 * // B. Map Meter/Resident Info dto.setBuildingName(meter.getBuildingName());
		 * dto.setBuildingId(meter.getBuildingId());
		 * dto.setResidentName(meter.getResidentName());
		 * dto.setFloorNo(meter.getFloorNo()); dto.setFlatNo(meter.getFlatNo());
		 * dto.setResidentType(meter.getResidentType());
		 * 
		 * // C. Map Address Info from Building Entity if
		 * (buildingMap.containsKey(meter.getBuildingId())) { BuildingEntity building =
		 * buildingMap.get(meter.getBuildingId());
		 * dto.setAddress(building.getAddress()); dto.setState(building.getState());
		 * dto.setCountry(building.getCountry()); } else { // Fallback if building ID
		 * exists in meter but not in building table dto.setAddress("N/A");
		 * dto.setState("N/A"); dto.setCountry("N/A"); }
		 * 
		 * filteredEvents.add(dto); } } } }
		 * 
		 * if (filteredEvents.isEmpty()) { return ResponseEntity.noContent().build(); }
		 * 
		 * return ResponseEntity.ok(filteredEvents);
		 * 
		 * } catch (Exception e) { e.printStackTrace(); return
		 * ResponseEntity.internalServerError().body("Error fetching tamper data: " +
		 * e.getMessage()); } }
		 */
