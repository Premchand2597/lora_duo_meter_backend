...
import com.loraDuoMeter.Entity.Recharge_Finish_Display_Entity;
import com.loraDuoMeter.Proj.IRechargeOverDisplayView;

@Repository
public interface IRecharge_Finish_Repo extends JpaRepository<Recharge_Finish_Display_Entity, Long> {
    ...
    @Query(value = """
        ...
        FROM recharge_finish_table d
        INNER JOIN (
            SELECT dev_eui, MAX(received_at) AS max_received
            FROM recharge_finish_table
            GROUP BY dev_eui
  ...
  TO_TIMESTAMP(d.last_update_time, 'DD-MM-YYYY HH24:MI:SS') DESC
        """, nativeQuery = true)
    Stream<IRechargeOverDisplayView> findLatestByDevEuiNative();
    ...
    @Query(value = """
    		...
    		""",
    		countQuery = """
    		SELECT COUNT(DISTINCT dev_eui) FROM recharge_finish_table
    		""",
    		nativeQuery = true)
    		Slice<IRechargeOverDisplayView> findByDevEui(Pageable pageable);
    ...
    @Query(value = """
            ...
            """,
            nativeQuery = true)
        Slice<IRechargeOverDisplayView> findLatestByDevEuiAndDevEui(
                @Param("devEui") String devEui,
                Pageable pageable
        );

    @Query(value = "SELECT COUNT(*) FROM recharge_finish_table d WHERE d.dev_eui = :devEui", nativeQuery = true)
    ...
    @Query(value = """
    	...
    	""",
    	nativeQuery = true)
    	Slice<IRechargeOverDisplayView> searchByDevEui(
    	        @Param("devEui") String devEui,
