...
import com.loraDuoMeter.Entity.Tamper_Event_Display_Entity;
import com.loraDuoMeter.Proj.ITamperEventDisplayView;

@Repository
public interface ITamper_Event_Repo extends JpaRepository<Tamper_Event_Display_Entity, Long> {
  ...
  TO_TIMESTAMP(d.last_update_time, 'DD-MM-YYYY HH24:MI:SS') DESC
        """, nativeQuery = true)
    Stream<ITamperEventDisplayView> findLatestByDevEuiNative();
    ...
    @Query(value = """
    		...
    		""",
    		countQuery = """
    		SELECT COUNT(DISTINCT dev_eui) FROM tamper_event_table
    		""",
    		nativeQuery = true)
    		Slice<ITamperEventDisplayView> findByDevEui(Pageable pageable);
    ...
    @Query(value = """
            ...
            """,
            nativeQuery = true)
        Slice<ITamperEventDisplayView> findLatestByDevEuiAndDevEui(
                @Param("devEui") String devEui,
                Pageable pageable
        );

    @Query(value = "SELECT COUNT(*) FROM tamper_event_table d WHERE d.dev_eui = :devEui", nativeQuery = true)
    ...
    @Query(value = """
    	...
    	""",
    	nativeQuery = true)
    	Slice<ITamperEventDisplayView> searchByDevEui(
    	        @Param("devEui") String devEui,
