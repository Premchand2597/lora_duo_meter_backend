...
import com.loraDuoMeter.Entity.Power_Up_Display_Entity;
import com.loraDuoMeter.Proj.IPowerUpDisplayView;

@Repository
public interface IPower_Up_Display_Repo extends JpaRepository<Power_Up_Display_Entity, Long> {
  ...
  TO_TIMESTAMP(d.last_update_time, 'DD-MM-YYYY HH24:MI:SS') DESC
        """, nativeQuery = true)
    Stream<IPowerUpDisplayView> findLatestByDevEuiNative();
    ...
    @Query(value = """
    		...
    		""",
    		countQuery = """
    		SELECT COUNT(DISTINCT dev_eui) FROM power_up_table
    		""",
    		nativeQuery = true)
    		Slice<IPowerUpDisplayView> findByDevEui(Pageable pageable);
    ...
    @Query(value = """
            ...
            """,
            nativeQuery = true)
        Slice<IPowerUpDisplayView> findLatestByDevEuiAndDevEui(
                @Param("devEui") String devEui,
                Pageable pageable
        );

    @Query(value = "SELECT COUNT(*) FROM power_up_table d WHERE d.dev_eui = :devEui", nativeQuery = true)
    ...
    @Query(value = """
    	...
    	""",
    	nativeQuery = true)
    	Slice<IPowerUpDisplayView> searchByDevEui(
    	        @Param("devEui") String devEui,
