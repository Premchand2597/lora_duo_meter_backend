...
import com.loraDuoMeter.Entity.Battery_Cut_Off_Entity;
import com.loraDuoMeter.Proj.IBatteryCutOffDisplayView;

@Repository
public interface IBattery_Cut_Off_Repo extends JpaRepository<Battery_Cut_Off_Entity, Long> {
    ...
    @Query(value = """
        ...
        FROM battery_cut_off_table d
        INNER JOIN (
            SELECT dev_eui, MAX(received_at) AS max_received
            FROM battery_cut_off_table
            GROUP BY dev_eui
  ...
  TO_TIMESTAMP(d.last_update_time, 'DD-MM-YYYY HH24:MI:SS') DESC
        """, nativeQuery = true)
    Stream<IBatteryCutOffDisplayView> findLatestByDevEuiNative();
    ...
    @Query(value = """
    		...
    		""",
    		countQuery = """
    		SELECT COUNT(DISTINCT dev_eui) FROM battery_cut_off_table
    		""",
    		nativeQuery = true)
    		Slice<IBatteryCutOffDisplayView> findByDevEui(Pageable pageable);
    ...
    @Query(value = """
            ...
            """,
            nativeQuery = true)
        Slice<IBatteryCutOffDisplayView> findLatestByDevEuiAndDevEui(
                @Param("devEui") String devEui,
                Pageable pageable
        );

    @Query(value = "SELECT COUNT(*) FROM battery_cut_off_table d WHERE d.dev_eui = :devEui", nativeQuery = true)
    ...
    @Query(value = """
    	...
    	""",
    	nativeQuery = true)
    	Slice<IBatteryCutOffDisplayView> searchByDevEui(
    	        @Param("devEui") String devEui,
