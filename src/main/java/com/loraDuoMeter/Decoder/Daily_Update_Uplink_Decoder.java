...
public class Daily_Update_Uplink_Decoder implements IPacketDecoder<Daily_Update_Uplink_Dto> {
    ...
    @Override
    public Daily_Update_Uplink_Dto decode(String hex) {
        Daily_Update_Uplink_Dto dto = new Daily_Update_Uplink_Dto();
        int i = 0;
     ...
     // ---- Device RTC (hh mm ss dd MM yy) ----     
        ...
        dto.setPending_event(hex.substring(i, i += 2));
      //  dto.setLast_update_time(now.format(fmt));

        // ---- Recharge status (1 byte) ----
        dto.setRecharge_status(hex.substring(i, i += 2));
